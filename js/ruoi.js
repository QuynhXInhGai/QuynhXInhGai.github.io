...
function Fly(fid, type, color, sh, sw, x, y, fly_img, name, title) {
		...
		setTimeout(function(){
			...
			switch (value_message) { 
				...
				case 5: 
					...
					$(t).html(parent.name + " nói:\n"+'"Đừng đập ta nhé cô nàng xinh xắn"')
					break;
				case 6: 
					$(t).html(parent.name + " nói:\n"+'"Đừng đập ta nhé cô nàng xinh xắn"')
					break;
	...
	f.addEventListener('click', function() {	
		if(hasswatter){
			...
			parent.stop();
			parent.died = true;
			f.title = "Click vào giữ 5s để hồi sinh em đi chị Huyền gái ơi...!";
		} else {
			if (Math.random() < 0.3) {
	...
	// }else{
		...
		setTimeout(function() {
			//if (Math.random() < show_percent/100) {
				...
				switch (value) { 
					case 1: 
						var fly = new Fly("test", "Nhặng", "Xanh", height_fly, width_fly, 0, 0, "https://redirect.nhq.vn/demo/js/images/ruoi_11.png", "Tiểu Cường", "Cổ Mộ");
						break;
					case 2:
					 	var fly = new Fly("test", "Ruồi", "Đen", height_fly, width_fly, 0, 0, "https://redirect.nhq.vn/demo/js/images/ly_mac_ruoi.png", "Tiểu Cường", "Cổ Mộ");						
					 	break;
					case 3: 
					 	var fly = new Fly("test", "Ruồi", "Đỏ", height_fly, width_fly, 0, 0, "https://redirect.nhq.vn/demo/js/images/doc_co_ruoi.png", "Tiểu Cường", "Tiêu Dao");		
					 	break;
					case 4: 
